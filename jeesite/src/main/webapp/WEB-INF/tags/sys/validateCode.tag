...
<div class="form-left">
<input type="text" id="${name}" name="${name}" maxlength="5" class="form-control" placeholder="验证码"/>
</div>
<div class="form-right">
<img src="${pageContext.request.contextPath}/servlet/validateCodeServlet" onclick="$('.${name}').attr('src','${pageContext.request.contextPath}/servlet/validateCodeServlet?'+new Date().getTime());" class="ccap mid ${name}" style="${imageCssStyle}"/>
</div>
