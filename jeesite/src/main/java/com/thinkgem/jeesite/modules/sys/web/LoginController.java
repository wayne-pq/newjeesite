...
//		view += ".jsp";
	...
	public String frontlogin(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		/* SecurityUtils.setSecurityManager(securityManager);  
		 ...
		 String username = WebUtils.getCleanParam(request, org.apache.shiro.web.filter.authc.FormAuthenticationFilter.DEFAULT_USERNAME_PARAM);
		 String password = WebUtils.getCleanParam(request, org.apache.shiro.web.filter.authc.FormAuthenticationFilter.DEFAULT_PASSWORD_PARAM);
			boolean rememberMe = WebUtils.isTrue(request, org.apache.shiro.web.filter.authc.FormAuthenticationFilter.DEFAULT_REMEMBER_ME_PARAM);
			boolean mobile = WebUtils.isTrue(request, FormAuthenticationFilter.DEFAULT_MOBILE_PARAM);
			...
			return new UsernamePasswordToken(username, password.toCharArray(), false, host, captcha, mobile);*/
		    //4、登录，即身份验证  
	      /*  subject.login(token);  */
		/*Principal principal = UserUtils.getPrincipal();
